...
|-------|----------|-------------------|
| **Provisioning** | `api.gimme.tools/userid/...` | All operations (authenticated to user) |
| **Active** | `userid.gimme.tools/...` | All operations per user-configured permissions |
...
| Token scope creep | Tokens audience-bound to `api.gimme.tools`, scoped to user |
| Session hijacking | Session cookies remain on `auth.gimme.tools` only |
...
- **Blast radius shrinks**: Compromise of one subdomain doesn't affect `api.gimme.tools` or other users
...
```bash
GET api.gimme.tools/user-123/.well-known/subdomain-status
```
