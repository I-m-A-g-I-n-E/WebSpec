...
- Only `auth.gimme.tools` can read the session cookie
- Services must call `auth.gimme.tools/verify` to validate
...
```yaml
location: auth.gimme.tools only (HttpOnly cookie)
lifetime: days/weeks
...
```
1. User authenticates at auth.gimme.tools
