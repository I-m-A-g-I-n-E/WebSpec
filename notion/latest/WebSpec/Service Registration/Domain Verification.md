...
- Register [`slack.gimme.tools`](http://slack.gimme.tools) without owning Slack
...
```
_[gimme-verify.acme.com](http://gimme-verify.acme.com)  TXT  "gimme-tools-verify=abc123xyz"
```
...
```
[https://acme.com/.well-known/gimme-tools-verify.txt](https://acme.com/.well-known/gimme-tools-verify.txt)
...
1. Service submits registration
              │
              ▼
2. [gimme.tools](http://gimme.tools) generates unique verification token
...
5. Service calls /services/verify
              │
              ▼
6. [gimme.tools](http://gimme.tools) checks all claimed domains:
...
```yaml
canonical_domains:
  - [acme.com](http://acme.com)           # Primary (required)
  - [app.acme.com](http://app.acme.com)       # Subdomain
  - [acme.io](http://acme.io)            # Alternative TLD
  - [legacy-acme.com](http://legacy-acme.com)    # Legacy domain
```
...
```yaml
canonical_domains:
  - [acme.com](http://acme.com)           # ✅ Verified via DNS
  - "*.[acme.com](http://acme.com)"       # ✅ Auto-verified (subdomain of [acme.com](http://acme.com))
  - [app.acme.io](http://app.acme.io)        # ✅ Verified via well-known file
  - [oldacme.net](http://oldacme.net)        # ❌ Not verified (registration proceeds without this domain)
```
...
```bash
GET [https://api.gimme.tools/services/{service_id}/verification](https://api.gimme.tools/services/{service_id}/verification)
...
```bash
POST [https://api.gimme.tools/services/{service_id}/verify](https://api.gimme.tools/services/{service_id}/verify)
Authorization: Bearer {admin_token}

{
  "domains": ["[acme.com](http://acme.com)"]  # Optional: specific domains, or omit for all
}
```
...
- All services reviewed by [gimme.tools](http://gimme.tools) team before activation
