...
- **Network:** Security is enforced by the Browser's Same-Origin Policy. [`drive.gimme.tools`](http://drive.gimme.tools) literally cannot read [`slack.gimme.tools`](http://slack.gimme.tools) cookies.
...
- A AI agent can use it (just standard tool calling)
